namespace System1Group.Lib.Attributes
{
    ...
    {
        public static IEnumerable<PropertyInfo> GetPropertiesImplementing<TAttribute>([AllowedToBeNull, DoesNotIntroduceCoupling] Type type)
            where TAttribute : Attribute
        {
            return type == null ? new List<PropertyInfo>() : type.GetProperties().Where(prop => Attribute.IsDefined(prop, typeof(TAttribute)));
        }

        public static IEnumerable<MethodInfo> GetMethodsImplementing<TAttribute>([AllowedToBeNull, DoesNotIntroduceCoupling] Type type)
            where TAttribute : Attribute
        {
            return type == null ? new List<MethodInfo>() : type.GetMethods().Where(meth => Attribute.IsDefined(meth, typeof(TAttribute)));
        }

        public static IEnumerable<Type> GetClassesImplementing<TAttribute>([AllowedToBeNull] IEnumerable<Type> types)
            where TAttribute : Attribute
        {
